...
bool dual_circle_push_back(DualCircleList *list, DualCircleListNode node)
{
    ...
    if (list && node)
    {
        if (0 == obj->m_length)
        {
            ...
            obj->m_head.next = node;
        }
        else
        {
            dual_circle_list_add(node, obj->m_head.next->pre, obj->m_head.next);
        }
        ++obj->m_length;
        ret = true;
    }
...
{
    return dual_circle_list_remove(list, ((dual_link_list_def *)list)->m_length - 1);
}
...
DualCircleList *dual_circle_list_create(list_create_t type, DualCircleList* list)
{
    ...
    if(ret)
    {
        ret->m_vtable = &s_dual_circle_list_vtable;
    }
